...
/**
 * Build CDN bundles for FeatherKit (simplified version)
 */
...
if (typeof window !== 'undefined') {
  ...
  // Verify components are registered after a short delay
  setTimeout(() => {
    if (!customElements.get('fk-button')) {
      console.warn('⚠️ fk-button not registered - decorator may have failed');
      // Try manual registration as fallback
      if (typeof FkButton !== 'undefined') {
        try {
          customElements.define('fk-button', FkButton);
          console.log('✅ fk-button manually registered');
        } catch (error) {
          console.error('❌ Failed to manually register fk-button:', error);
        }
      }
    } else {
      console.log('✅ fk-button registered successfully');
    }
    
    if (!customElements.get('fk-icon')) {
      console.warn('⚠️ fk-icon not registered - decorator may have failed');
      // Try manual registration as fallback
      if (typeof FkIcon !== 'undefined') {
        try {
          customElements.define('fk-icon', FkIcon);
          console.log('✅ fk-icon manually registered');
        } catch (error) {
          console.error('❌ Failed to manually register fk-icon:', error);
        }
      }
    } else {
      console.log('✅ fk-icon registered successfully');
    }
  }, 100);
}
`;
...
async function generateCSSBundle(cdnDir) {
  ...
  // Try to read theme CSS files
  let lightThemeCSS = '';
  try {
    lightThemeCSS = readFileSync(join(themesPath, 'light.css'), 'utf8');
  } catch (error) {
    console.log('  ⚠️  Light theme CSS not found, using fallback');
    lightThemeCSS = generateFallbackLightTheme();
  }
...
/**
 * Generate fallback light theme CSS
 */
function generateFallbackLightTheme() {
  return `
:root {
  /* Color tokens */
  --fk-color-primary-50: #f0f9ff;
  --fk-color-primary-100: #e0f2fe;
  --fk-color-primary-200: #bae6fd;
  --fk-color-primary-300: #7dd3fc;
  --fk-color-primary-400: #38bdf8;
  --fk-color-primary-500: #0ea5e9;
  --fk-color-primary-600: #0284c7;
  --fk-color-primary-700: #0369a1;
  --fk-color-primary-800: #075985;
  --fk-color-primary-900: #0c4a6e;
  --fk-color-primary-950: #082f49;
  
  --fk-color-secondary-50: #f8fafc;
  --fk-color-secondary-100: #f1f5f9;
  --fk-color-secondary-200: #e2e8f0;
  --fk-color-secondary-300: #cbd5e1;
  --fk-color-secondary-400: #94a3b8;
  --fk-color-secondary-500: #64748b;
  --fk-color-secondary-600: #475569;
  --fk-color-secondary-700: #334155;
  --fk-color-secondary-800: #1e293b;
  --fk-color-secondary-900: #0f172a;
  --fk-color-secondary-950: #020617;
  
  --fk-color-error-500: #ef4444;
  --fk-color-error-600: #dc2626;
  --fk-color-error-foreground: #ffffff;
  --fk-color-success-500: #22c55e;
  --fk-color-warning-500: #f59e0b;
  
  --fk-color-background: #ffffff;
  --fk-color-foreground: #0f172a;
  --fk-color-card: #ffffff;
  --fk-color-border: #e2e8f0;
  --fk-color-muted: #f8fafc;
  --fk-color-muted-foreground: #64748b;
  
  /* Spacing tokens */
  --fk-spacing-1: 0.25rem;
  --fk-spacing-2: 0.5rem;
  --fk-spacing-3: 0.75rem;
  --fk-spacing-4: 1rem;
  --fk-spacing-6: 1.5rem;
  
  /* Radius tokens */
  --fk-radius-sm: 0.125rem;
  --fk-radius-md: 0.375rem;
  --fk-radius-lg: 0.5rem;
  
  /* Typography tokens */
  --fk-typography-font-family-sans: ui-sans-serif, system-ui, sans-serif;
  --fk-typography-font-size-sm: 0.875rem;
  --fk-typography-font-size-base: 1rem;
  --fk-typography-font-size-lg: 1.125rem;
  --fk-typography-font-weight-medium: 500;
  --fk-typography-line-height-normal: 1.5;
  
  /* Motion tokens */
  --fk-motion-transition-base: 200ms cubic-bezier(0, 0, 0.2, 1);
  
  /* Shadow tokens */
  --fk-shadow-md: 0 4px 6px -1px rgb(0 0 0 / 0.1);
}
`;
}
